...
class RequestController {
	static async getMyRequests(req, res) {
		if (req.session.isWorker) {
			RequestModel.getRequestByWorkerId(req.session.client.client_id).then((data) => {
				res.render('pages/index', {title: "Ваши запросы", requests: data.rows, isWorker: req.session.isWorker})
			})
			return
		}

		RequestModel.getRequestByClientId(req.session.client.client_id).then((data) => {
			res.render('pages/index', {title: "Ваши запросы", requests: data.rows, isWorker: req.session.isWorker})
		})
	}
	...
	static async getOneRequest(req, res) {
		DocumentsModel.getAllByRequestId(req.params.request_id).then((documents) => {
			const formattedDocuments = documents.rows.map((doc) => {
				return {
					...doc,
					create_time: formatDate(doc.create_time)
				}
			})
			RequestModel.getRequestById(req.params.request_id).then((request) => {

				const formatedRequest = {
					...request.rows[0],
					create_time: formatDate( request.rows[0].create_time),
					status_update_time: formatDate( request.rows[0].status_update_time),
					close_time: formatDate( request.rows[0].close_time),
				}
	...
	static async updateFinishRequest(req, res) {
		RequestModel.updateFinishRequestById(req.params.request_id).then(() => {
			res.redirect(`/requests/${req.params.request_id}`)
		})
	}

	static async getUpdateWorkerRequest(req, res) {
		RequestModel.getRequestById(req.params.id).then((request) => {
			WorkerModel.getWorkersByDepartmentId(request.rows[0].department_id).then((workers) => {
	...
	static async updateWorkerRequest(req, res) {
		RequestModel.updateWorkerRequestById(req.params.request_id, req.body.worker).then(() => {
			res.redirect(`/requests/${req.params.request_id}`)
		})
	}

	static async deleteRequest(req, res) {
		RequestModel.deleteRequestById(req.params.request_id).then(() => {
			res.redirect('/')
		})
	}
}
